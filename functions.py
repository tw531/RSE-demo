...
def print_outputs (Status_Mode,Status_EmpPred,Status_HrentPred,Output,Hrent,Tol,Project_Name):
    ...
    T_IJ.columns = ['z1','z2','z3']
    T_IJ.index = pd.MultiIndex.from_tuples([('high income', 'z1'), ('high income', 'z2'), ('high income', 'z3'), 
                                            ('low income', 'z1'), ('low income', 'z2'), ('low income','z3')])
    
    T_IJ_all = pd.DataFrame(sum([Output['IJ'][l] for l in list(range(0,Output['IJ'].shape[0]))]))
    T_IJ_all.columns = ['z1','z2','z3']
    T_IJ_all.index = ['z1','z2','z3']
    
    
    T2 = pd.concat([T_IJ, T_IJ_all], axis=0)
    ...
    T1.to_excel(Filename, sheet_name='Hrent_EREW_Tran')
    T2.to_excel(Filename, sheet_name='Commuting_OD_Flow')
...
#                       'T_Tran':T_Tran,
                      'T1':T1,
                      'Commuting_Origin_Destination_Flow':T2}
